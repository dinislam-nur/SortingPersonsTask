...
class SortingPersonsByQuickSortTest {
    ...
    void sortTest() {
        ...
        //Execution
        final PersonSorter sorter = new SortingPersonsByQuickSort();
        sorter.sort(persons);
    ...
    private List<String> getListName(String nameResourcePath) {
        ...
        try (BufferedReader bfr = new BufferedReader(
                new InputStreamReader(
                        SortingPersonsByQuickSort.class.getResourceAsStream(nameResourcePath)
                ))) {
            while (bfr.ready()) {
